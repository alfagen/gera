...
Система смотрит, какую комиссию ставят конкуренты на этих позициях, и ставит **на 0.0001% ниже** первого подходящего — чтобы быть чуть выгоднее.
...
4. Берется первый обменник, из его комиссии вычитается `AUTO_COMISSION_GAP` (0.0001)
...
|-----------|----------|------|----------|
| `AUTO_COMISSION_GAP` | 0.0001 | autorate_calculators/base.rb | Отступ от комиссии конкурента |
