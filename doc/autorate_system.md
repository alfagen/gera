...
4. Берется первый обменник, из его комиссии вычитается `AUTO_COMISSION_GAP` (0.0001, т.е. 0.01%)
...
|-----------|----------|------|----------|
| `AUTO_COMISSION_GAP` | 0.0001 | rate_comission_calculator.rb | Отступ от комиссии конкурента (0.01%) |
...
3. **Расчет `auto_comission_by_external_comissions`:**
   ...
   - Первый: 2.5%
   - Результат: 2.5% - 0.01% = **2.49%** (в коде: 2.5 - 0.0001 = 2.4999)
