...
class CBRRatesWorker
  ...
  def fetch_rates date
    if GERA::CbrExternalRate.where(date: date, cur_from: CURRENCIES.map(&:iso_code)).count == CURRENCIES.count
    ...
    CURRENCIES.each do |cur|
      if GERA::CbrExternalRate.where(date: date, cur_from: cur.iso_code).exists?
        logger.info "Данные по валюте #{cur} на дату #{date} уже есть в базе"
      else
        save_rate get_rate(root, CBR_IDS[cur]), cur, date
      end
    end
  end
