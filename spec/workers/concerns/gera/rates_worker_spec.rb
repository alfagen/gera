...
module Gera
  RSpec.describe RatesWorker do
    # Create a test worker class that includes RatesWorker
    ...
    before do
      worker.test_rate_source = rate_source
    end
    ...
    describe '#perform' do
      context 'with valid rates' do
        ...
        it 'creates a rate source snapshot' do
          expect { worker.perform }.to change(ExternalRateSnapshot, :count).by(1)
        end
        ...
        it 'enqueues ExternalRatesBatchWorker' do
          expect(ExternalRatesBatchWorker).to receive(:perform_async)
      ...
      context 'with array-based rate data' do
        ...
        let(:array_worker) { array_worker_class.new }

        before do
          array_worker.test_rate_source = rate_source
          array_worker.test_rates = {
            'BTC/USD' => [nil, nil, nil, nil, nil, nil, nil, 50000.0]
          }
        end

        it 'handles array-based rate data' do
          expect { array_worker.perform }.to change(ExternalRateSnapshot, :count).by(1)
        end
      end
    end
